function [mdice,mnmi,mcv]=cross_validation_mod_L_test2(wd,prefix,part,sub_list,kc,iter,prob_thre,mpm_thre)
...
for ti=1:N
    ...
    mpm_cluster1=cluster_mpm_mod_L(wd,prefix,part,list1_sub,kc,prob_thre,mpm_thre);
    mpm_cluster2=cluster_mpm_mod_L(wd,prefix,part,list2_sub,kc,prob_thre,mpm_thre);
    ...
    for ki=1:kc
        ...
        den=den+length(find(tmp1>0))+length(find(tmp2>0)); % modified
...
fp=fopen(strcat(wd,'/validation/',part,'_L_stability_index_test_2.txt'),'at');
if fp
    ...
    %fprintf(fp,'\n');
	fprintf(fp,'mdice: %f',mean(dice(:,2))/2);
    fprintf(fp,'\n');
	fprintf(fp,'\n');
end
fclose(fp);
